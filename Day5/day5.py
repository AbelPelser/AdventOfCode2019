...
class Program:
    def __init__(self, memory: List[int], noun: int = None, verb: int = None):
        ...
        self.opcode_map: Dict[int, FunctionType] = {
            1: self.add,
            2: self.multiply,
            3: self.input_int,
            4: self.output_int,
    ...
    def transform_params(self, params: List[int], param_modes: List[int]):
        assert len(params) == len(param_modes)
    ...
    def handle_opcode(self, opcode):
        ...
        func = self.opcode_map[func_code]
